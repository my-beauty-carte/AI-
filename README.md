...
2. 録音停止後、以下を**すべて自動で**実行
   ...
   - スライド(.pptx)の生成(各スライドにシンプルなイラストも自動生成・配置)
   - スライド + 音声ナレーション(自然な音声によるOpenAI TTS)による振り返り動画(.mp4)の生成
   - 生成した動画をフルスクリーンで自動再生
...
- APIキー
  ...
  - `OPENAI_API_KEY`(文字起こし、音声ナレーション、スライドのイラスト生成)

音声ナレーションとイラスト生成にOpenAIのAPIを使うため、以前より少しAPI利用料が
かかります(セミナー1回・スライド6〜10枚程度なら、目安として数十円〜百円程度)。
